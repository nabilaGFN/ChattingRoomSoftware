...
class Receive(threading.Thread):
    ...
    def run(self):
        while True:
            message = self.recv(1024).decode('ascii')
            if message:
                if self.messages:
...
class Client:
    ...
    def start(self):
        ...
        send.start()
        receive.start()
        self.sock.sendall('Server : {} has joined th chat. say whatsup!'.format(self.name).encode('ascii'))
    ...
    def main(host , port):
        ...
        btnSend.grid(row=0,column=1,pady=10,sticky="ew")
